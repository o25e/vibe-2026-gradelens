...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const assignment = db.prepare('SELECT * FROM assignments WHERE id = ?').get(params.id) as Record<string, unknown> | undefined
  if (!assignment) return NextResponse.json({ error: 'Not found' }, { status: 404 })
  ...
  const updated = db.prepare('SELECT * FROM assignments WHERE id = ?').get(params.id)
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  db.prepare('DELETE FROM grade_settings WHERE assignment_id = ?').run(params.id)
  ...
  `).run(params.id)
  db.prepare('DELETE FROM submissions WHERE assignment_id = ?').run(params.id)
  db.prepare('DELETE FROM rubric_items WHERE assignment_id = ?').run(params.id)
  db.prepare('DELETE FROM assignments WHERE id = ?').run(params.id)
